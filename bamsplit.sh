...
awk -v short=$out_short -v long=$out_long 'BEGIN{OFS="\t"}/^#/{next}(NR==FNR){if($9<30||$10<30){next}if($2!=$4){next}if($5-$3<=1000&&$6=="+"&&$7=="-"){a[$1]++;next}if($5-$3>=2000){b[$1]++;next}}/^@/{print $0 > short; print $0 > long ;next}{$2+=2}($1 in a){print $0 > short}($1 in b){print $0 > long}' $pairs ${bam}.sam 

#awk -v short=$out_short -v long=$out_long 'BEGIN{OFS="\t"}/^#/{next}{$2+=2}(NR==FNR){if($9<30||$10<30){next}if($2!=$4){next}if($5-$3<=1000&&$6=="+"&&$7=="-"){next}{b[$1]++;next}}/^@/{ print $0 > long ;next}($1 in b){print $0 > long}' $pairs ${bam}.sam
